...
void loop() {
  ...
  while(page == 3) {
    ...
    float voltage_test_result = voltage_startup*10; // Equal to voltage test value * 10 since 1 to 10 ratio (9k <-> 1k resistor) so the actual voltage is the voltage * 10
    ...
    if((voltage_test_result < 1 && voltage_test_result > 0) || (voltage_test_result > -1 && voltage_test_result < 0))
    {
    voltage_test_result = voltage_test_result*1000;
...
while(page == 4) {
  ...
  if(sensitivity_value > 30)
  sensitivity_value = 10; //Back to 0.185
  
  
  if(sensitivity_value == 10)
  _sensitivity = 0.185;
  if(sensitivity_value == 20)
  _sensitivity = 0.100;
  if(sensitivity_value == 30)
  _sensitivity = 0.066;
  ...
  current = (voltage - 2.5) / _sensitivity;
    ...
    display.print("ADC Value/Voltage:"); 
    display.setCursor(0,10);  
  ...
  if(unknown_voltage < 4.7) 
  {
    ...
    display.print(Rx); 
    display.print(pPUnit);  //print out pPunit
    display.print("ohm");
    display.setCursor(0,20); 
    display.print("Connect C and D");
    delay(50);
...
while(page == 6) {
    ...
    display.setCursor(0, 20);  
    display.println("Status"); //Short Circuit Function
    while(1)
    {
    if(shortport == 0)
    {
    display.setCursor(0, 30);  
    display.println("Not Detected"); //Short Circuit Function
    }

    if(shortport == 1)
    {
    display.setCursor(0, 30);  
    display.println("Short Circuit"); //Short Circuit Function
    }
